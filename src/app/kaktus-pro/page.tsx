...
export default function KaktusProPage() {
  ...
  const socials = (["instagram", "facebook", "tiktok", "telegram", "whatsapp"] as const)
    ...
    <div className="min-h-svh bg-gradient-to-b from-[#1a3a6e] via-[#14305e] to-[#0b1f42] flex flex-col items-center justify-center px-5 py-8 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
            ...
            {(["az", "en", "ru"] as Locale[]).map((lang) => {
              ...
              return (
                <button key={lang} onClick={() => setLocale(lang)}
                  className={`p-1.5 rounded-md transition-all ${locale === lang ? "bg-white/20" : "opacity-40 hover:opacity-70"}`}>
              ...
              <div className="flex items-center gap-2 mt-3 pt-3 border-t border-white/[0.08]">
                {socials.map((s) => (
                  <a key={s.key} href={s.href} target="_blank" rel="noopener noreferrer"
                    className="w-8 h-8 rounded-lg bg-white/[0.06] hover:bg-white/15 flex items-center justify-center text-white/40 hover:text-white transition-all">
                    {s.icon}
                  </a>
                ))}
              </div>
        ...
        <p className="text-center text-white/20 text-[10px] tracking-wider">
          Powered by <a href="https://burncode.org" target="_blank" rel="noopener noreferrer" className="text-white/30 hover:text-white transition-colors">Burncode LLC</a>
        </p>
      </div>
    </div>
  );
